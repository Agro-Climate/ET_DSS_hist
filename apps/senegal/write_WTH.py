...
def write_WTH(df_tab, df_gen, WTD_fname, Wdir_path):  
    ...
    PltDate = int(plt_year + repr(plt_doy).zfill(3))  #integer  #e.g. 2015310   # September 15, 2015   ********
    hv_date = PltDate + 210 #tentative harvest date => long enough considering delayed growth  
    ...
    frst_date = find_frst_date(PltDate, hv_date, trimester1)  #frst_date in format YYYYDOY[int]
    IC_date = PltDate - 1  #for weather generation for IC => IC starts 1 days before the first planting date
    #adjust IC_date if initial conditionn belong to the previous year
    if PltDate%1000 == 1 : #<= 30:
        IC_date = (PltDate//1000 -1)*1000 + 365
        if calendar.isleap(PltDate//1000 -1):  IC_date = (PltDate//1000 -1)*1000 + 366

    #adjust hv_date if harvest moves to a next year
    yr_end_date = 365
    if calendar.isleap(PltDate//1000): yr_end_date = 366
...
#============here
    ...
    # if PltDate//1000 == hv_date//1000 and PltDate//1000 == IC_date//1000 and SCF1_loc[0][0] != 12:
    ...
    #(b) write observed weather until frst_date and then WGEN generated weather realizations 
        ...
        if WTD_last_date >= frst_date:  
            ...
            print('No case selected in writing WTH: PltDate//1000 == hv_date//1000 and PltDate//1000 == IC_date//1000')
            os.system('pause')
...
def write_WTH_obs_ensemble(IC_date, frst_date1, WTD_df, df_gen, Wdir_path, LAT, LONG, ELEV, TAV, AMP, sce_name):
    ...
    for ii in range(0,nrealiz):
        ...
        if calendar.isleap(iyear):  
            ...
            if calendar.isleap(plt_year):
                #=========write header
                fout_WTH = path.join(Wdir_path, sce_name.upper() + repr(plt_year+ii)[2:4]+'01.WTH') #Linux looks for all capital for WTH name
                f_rz = open(fout_WTH,"w") #opens file
                f_rz.write("*WEATHER DATA : "+sce_name + "\n")
                f_rz.write(" \n")
                f_rz.write("@ INSI      LAT     LONG  ELEV   TAV   AMP REFHT WNDHT \n")
...
def find_station_info(station_name):
  station_dic = {
    ...
    'KOLD': {'LAT': 12.883, 'LONG':-14.25, 'ELEV': 22, 'TAV': 27.2, 'AMP': 6.9},  #Ziguinchor
        ...
        # 'ZIGU': {'LAT': 12.567, 'LONG':-16.267, 'ELEV': 10, 'TAV': 27.2, 'AMP': 6.9},  #Ziguinchor
...
def find_frst_date(PltDate, hv_date, trimester):  
  # #===================================================
...
# https://docs.google.com/spreadsheets/d/1XnBcHzbTDtDJpHl9U9SX0AfNgDjy4Vci/edit#gid=520906445
  if trimester == 'JFM':  #=> JFM is always in YR2 and thus use harvest date as reference => Case #6
    if calendar.isleap(hv_date//1000):
      ...
      frst_date = repr(PltDate//1000) + repr(temp_doy).zfill(3)  
  else:
    if calendar.isleap(PltDate//1000):
      temp_doy = frst_dic_leap.get(trimester)
      frst_date = repr(PltDate//1000) + repr(temp_doy).zfill(3)  
    else:
      temp_doy = frst_dic.get(trimester)
